...
export default function PayNowPage({ params }) {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-white p-6 rounded-xl shadow-md space-y-6">
      ...
      {selectedMethod === "mastercard" && (
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <input type="text" placeholder="Expiration date" className="input w-1/2" />
            <input type="text" placeholder="CVC" className="input w-1/2" />
          </div>
        </div>
      )}
      ...
      {/* Pay Now button */}
      {selectedMethod && (
        <button className="w-full py-3 text-white font-semibold bg-gradient-to-r from-purple-500 to-blue-500 rounded-md hover:opacity-90 transition">
          Pay now
        </button>
      )}
    </div>
